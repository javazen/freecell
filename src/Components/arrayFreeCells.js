...
export default function ArrayFreeCells({deal}) {
    const nullCard = 'back';
    const transform = "scale(0.6)";
    const cardsInFreeCells = deal.cardsInFreeCells;
    if (cardsInFreeCells.length != 4) console.log('ArrayFreeCells: cardsInFreeCells should have 4 elements');

    const cardObjects = cardsInFreeCells.map( function(o, i) {return {x:200*i, y:30, card:adj(o), href:href(o)}}  );
    ...
    return ( 
        <div className="free-cell-rect-array">
            <svg 
            width="500"
            height="250"
